...
const Navbar: React.FC = () => {
  ...
  return (
    <motion.nav
      initial={{ y: -100 }}
      animate={{ y: 0 }}
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'glass shadow-lg border-none' : 'bg-white/80 backdrop-blur-sm border-none'
        }`}
    >
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              ...
              >
                ...
                <motion.div
                  className="absolute -bottom-1 left-0 right-0 h-0.5 bg-gradient-teal-pink"
                  initial={{ scaleX: 0 }}
                  whileHover={{ scaleX: 1 }}
          ...
          <div className="hidden md:block">
            <a href="mailto:manatech.services.official@gmail.com?subject=Book a Call Request&body=Hi, I would like to schedule a call to discuss my project.">
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="px-6 py-2.5 bg-black text-white font-semibold rounded-full glow transition-all duration-300"
              >
                Book a Call
              </motion.button>
            </a>
