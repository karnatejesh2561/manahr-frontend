    ...
    <svg viewBox={`0 0 ${size} ${size}`} className="w-full h-auto">
      ...
      {centerLabel && (
        <text x={size / 2} y={size / 2 - 4} textAnchor="middle"
              fill="white" fontSize="22" fontFamily="Instrument Serif, Georgia, serif">
          {centerLabel}
        </text>
      )}
      {centerSubLabel && (
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} className="w-full h-auto">
      ...
      <text x={size / 2} y={size / 2 - 2} textAnchor="middle"
            fill="white" fontSize="28" fontFamily="Instrument Serif, Georgia, serif">
        {Math.round(value)}%
      </text>
      {label && (
      ...
      {stages.map((s, i) => {
        ...
        return (
          <div key={i} className="relative">
            ...
            <div className="h-9 rounded-lg overflow-hidden" style={{ background: 'rgba(255,255,255,0.04)' }}>
              <motion.div
