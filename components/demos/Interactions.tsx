    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          />
          <motion.div
            className="fixed left-1/2 top-1/2 z-50 sc-card p-6"
            style={{
              width: 400, maxWidth: '90vw', transform: 'translate(-50%, -50%)',
              color: 'var(--sc-text)',
              fontFamily: "'JetBrains Mono', ui-monospace, monospace",
            }}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 bottom-full mb-1 z-30 rounded-xl overflow-hidden"
            style={{
              minWidth: 160,
              background: 'rgba(255,255,255,0.85)',
        ...
        <AnimatePresence>
          {items.map(t => (
            <motion.div key={t.id}
              ...
              animate={{ opacity: 1, y: 0, scale: 1 }}
              exit={{ opacity: 0, x: 40 }}
              className="sc-card px-4 py-3 flex items-center gap-3"
              style={{
                minWidth: 260,
